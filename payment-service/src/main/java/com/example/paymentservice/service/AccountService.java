...
public class AccountService {
    ...
    public Account deposit(String name, String email, double amount) {
        ...
        Optional<Account> accountOptional = accountDao.findAccountByEmail(email);
        ...
        if (accountOptional.isPresent()) {
            ...
            Account account = new Account(name, email, amount, LocalDateTime.now());
    ...
    public double getBalance(String email) {
        return accountDao.getBalance(email).orElseThrow(AccountNotFoundException::new);
    }

    public Account withdraw(String name, String email, double amount) {

        Account account = accountDao.findAccountByEmail(email).orElseThrow(AccountNotFoundException::new);
    ...
    public void register(Customer customer) {
        try {
            this.restTemplate.postForEntity("http://localhost:8080/security/register",customer, String.class);
        }
        catch (Exception e){
